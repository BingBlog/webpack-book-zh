...
[PurifyCSS](https://github.com/purifycss/purifycss)是一个通过分析文件来实现这个目的的插件。它遍历你的代码，找出哪些CSS被是用了。大多数时候，可以找到足够的信息来删除项目中未被使用的代码。在某种程度上，它也适用于单叶应用程序。
...
为了使我们的项目更加真实，我们安装一个小型框架[Pure.css](http://purecss.io/)，我们在项目中引用这个框架，是我们可以看到PurifyCSS的效果。这两个项目除了名字相似外，并没有其它关系。
...
我们还需要是我们项目中的组件使用一个`Pure.css`的样式类，所以我们可以如下修改：
...
正如你所见，CSS文件的大小有所增加。我们将用PurifyCSS来修正这个问题。
...
使用PurifyCSS可以显着节省成本。 在他们的例子中，他们在一个应用程序中使用大约40％的选择器来纯化和缩减Bootstrap（140 kB），直到约35 kB。 这是一个很大的区别。
...
样式文件的体积明显减少了。有之前的16k到现在的2k。这个差距对于大型CSS框架来说将会更大。
...
PurifyCSS支持[附加选项]（https://github.com/purifycss/purifycss#the-optional-options-argument），包括`minify`。 您可以在实例化插件时通过`purifyOptions`字段启用它们。 给定PurifyCSS可能不会选择您正在使用的所有类，您应该使用`purifyOptions.whitelist`数组来定义它应该留在结果中的选择器。
...
W> 因为它的工作方式,使用PurifyCSS将丢失CSSsource maps，即使您已通过加载器特定配置启用它们。
...
使用PurifyCSS可以使文件大小显着减少。 它对依赖于重CSS框架的静态网站特别有用。 站点或应用程序变得越动态，就越难以可靠地分析。
...
* 最多，PurifyCSS可以消除大多数（如果不是全部）未使用的CSS规则。
...
* 关键路径渲染是另一种CSS技术，强调首先渲染上面的CSS。 这个想法是尽可能快地渲染，而不是等待所有的CSS加载。
...
我们的演示项目的样式部分现在已经初显形状了。 我们可以通过在项目中支持CSS语法检测来使开发更加容易。 下一章节，我们将实现这个。