...
目前的方案让我们无法缓存CSS。在某些情况下，我们可能会遭受**Flash of Unstyled Content** (FOUC)。FOUC的发生是因为浏览器加载JavaScript需要花费一定的时间，当我们把样式內联到了JavaScript当中，只有JavaScript加载完毕后，样式才会生效。分离CSS到单独的文件，可以让浏览器分别处理它们，进而避免这个问题。
...
由于这个处理过程，`ExtractTextPlugin`在编译阶段带来很大的开销。它在设计上不能同Hot Module Replacement (HMR)一起使用。由于我们仅会在生产环境中使用它，所以也没有什么影响。
...
`ExtractTextPlugin`包含了一个加载器。`ExtractTextPlugin.extract`标记了将要被提取的资源。插件将基于此执行其工作。
...
`ExtractTextPlugin.extract`接受`use` 和 `fallback`两个定义项。`ExtractTextPlugin`默认通过`use`来处理**初始化代码集成块**的内容，使用`fallback`处理剩下的内容。
...
重要的是需要注意：如果涉及到更多的格式，如从Sass提取CSS，你将不得不传递多个加载器到`use`选项。 `use`和`fallback`接受一个加载器（字符串）或一个加载器定义或者一个加载器定义数组。
...
`[name]`占位符将使用CSS引用条目的名称。 占位符和整体思路在*添加哈希到文件名*一章中详细讨论。
...
T> 如果从单独入口文件中提取多个CSS文件，可以使用[merge-files-webpack-plugin](https://www.npmjs.com/package/merge-files-webpack-plugin)将它们连接成一个文件。
...
如此设置，我们仍然可以在开发过程中，享受热加载的好处。 对于生产环境的构建打包，我们生成一个单独的CSS。 *html-webpack-plugin*会自动获取，并将最终的包注入我们的`index.html`文件中。
...
T> 如果你使用了CSS模块化特性，请记住按照*加载样式*章节中的讨论，相应地调整`use`配置项。 您可能还需要对普通CSS和CSS模块分别维护单独的设置，以便通过单独的逻辑加载它们。
...
现在我们的样式已经输出到了一个单独的CSS文件中。 因此，我们的JavaScript包变得稍微小了一些。浏览器不必等待JavaScript加载以获取样式信息。它可以单独处理CSS，避免FOUC问题。
...
如果您必须迁移依赖于CSS连接的旧项目，该方法可能很有用。 如果你想要严格控制排序，你可以设置一个单独的CSS入口，然后使用`@import`把剩余的项目通过它引入。 另一个选项是设置一个JavaScript入口并通过`import`获得相同的效果。
...
* 对样式使用`ExtractTextPlugin`解决了（FOUC）的问题。 将CSS与JavaScript分离还可以提高缓存行为并消除潜在的被攻击因素。
