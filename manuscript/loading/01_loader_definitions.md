...
使用绝对路径是一个好主意，可以让你在不破坏假设的情况下移动配置文件。另一个选项就是设置`context`字段，可以实现类似的效果，可以影响入口文件和加载器被处理的方式。但是对输出不会有任何影响，所以，仍然需要在设置输出时使用绝对路径或者`/`。
...
假设你设置了一个`include`或者`exclude`规则，从*node_modules*加载的代码包仍然会按照一个假设来处理，这个假设是它们已经被编译了，可以直接使用。有时你可能会遇到一个没有很好打包的代码，但是你可以通过调整加载器配置或者对包含在这个不匹配规则的代码包中的资源设置`resolve.alias`来解决问题。
...
Webpack通过*加载器*可以支持各种各样的格式。他还支持好几种格式的JavaScript模块化直接被使用。基本思想是相同的，设置加载器或者多个加载器，并将它们同你的目录结构联系起来。

{pagebreak}

Consider the example below where webpack is set to process JavaScript through Babel:

考虑如下的例子， 设置webpack使其通过Babel来处理JavaScript：
...
需要牢记，webpack的加载器总是按照从右到左、从下到上（单独定义）的顺序被执行。对于从右到左的规则，当你将其视为函数时，更容易记住。根据这条规则，你可以将定义`use: ['style-loader', 'css-loader']`读作`style(css(input))`。
...
即使使用上述规则就可以开发出任何配置，在普通的规则之前或之后可以强制应用一些特定的规则也是很方便的。`enforce`字段可以派上用场。它可以设置为`pre` or `post`，以在其它加载器之前或之后推送处理（特定的规则）。
...
在之前的*Linting JavaScript*章节已经使用过这个主意。语法检测（Linting）是一个很好的例子，因为构建应该在其执行其它任务之前就报错。`enforce: 'post'`的应用很少，它意味着你想对构建的代码进行检查。对构建代码进行分析可能是一个例子。
...
如果你将这个声明同其它和`test`字段相关的加载器小心地串联到一起，也可以编写出相同的配置而不是用`enforce`。使用`enforce`免除了这个必要，让你可将加载器的执行拆分为不同的阶段，进而可以更容易的进行组合。
...
因为webpack可以识别，这种风格的配置在入口和资源引入时同样有效。这种格式在一些个别的特定的情况下很有用，但通常情况下，你最好还是使用更易读的方式。

It's preferable to use the combination of `loader` and `options` fields:

最好是将`loader` 和 `options`结合起来使用：
...
在本书的设置中，你以很高的水平组合了配置。另外一个可以实现相同结果的选项是在`use`字段中分枝，因为webpack的加载器定义可以接受函数，进而允许你根据环境进行分枝。
...
这种方式的问题就是它将你的资源和webpack进行了挂钩。但仍然值得了解。由于入口配置以同样的机制运行，所以相同格式的配置也可以运用到那里:
...
加载器的行为可以通过检查来更详细的理解。[loader-runner](https://www.npmjs.com/package/loader-runner) 可以让你不用webpack独立运行它们。webpack在内部使用这个代码包，*Extending with Loaders*章节更详细的涵盖了这个话题。
...
[inspect-loader](https://www.npmjs.com/package/inspect-loader)让你可以检查在加载器之前传递的数据。不必在*node_modules*中插入`console.log`，您可以将此加载器附加到您的配置中，然偶胡并检查其中的流程。
...
由于webpack 2禁止随意使用基于根目录的配置，你需要使用`LoaderOptionsPlugin`。这个插件是为了遗留的兼容性问题而存在的，在为了的发布中将会被去掉。考虑如下的例子:
...
Webpack提供了多种方式来设置加载器，但是在webpack2中仅使用`use`就足够了。需要注意加载器的顺序，因为它是最常见问题根源。
...
* 一个加载器定义包含基于什么将会被匹配的**条件**和当匹配发生后将会被执行的**行动**。
...
* `LoaderOptionsPlugin`因遗留问题而存在，允许你处理webpack2中严格的配置模式
